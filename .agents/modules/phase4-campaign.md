...
Typo: `url_youtobe`, `url_usp`, `url_yotobe` (create extra), `GetCampaignRunProfile_Genaral`, `default_campaign_id` TV `int.parse` → gửi `'0'`. Trailing `/1` = chỉ approved_yn=1.
...
- Lịch TV: 1 query camp theo dir + 1 query time_runs `whereIn campaign_id` (không N+1).
